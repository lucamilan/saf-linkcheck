...
The same discipline binds questions about the **current code**, which is indexed and graphed alongside the cognition: `recall` over the code scope locates it, and the code-graph reports answer relations between symbols — who calls this, what a change reaches. Their answer is derived from the graph, so an empty one is a result, not a failed search. Text search over files is an anchor once the engine has answered, never the opening move. Declare the outcome here too, before concluding that something exists, is unused, or is safe to change.
